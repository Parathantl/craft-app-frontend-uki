...
export const ProductProvider = ({ children }) => {
  ...
  const toggleProductStatus = async (id) => {
    try {
      const response = await axios.patch(`http://localhost:4000/api/products/${id}/toggle-status`);
      dispatch({ type: 'UPDATE_PRODUCT', payload: response.data });
      return { success: true };
    } catch (error) {
  ...
  const fetchCreatorProducts = async (page = 1) => {
    dispatch({ type: 'FETCH_PRODUCTS_START' });
    try {
      const response = await axios.get(`http://localhost:4000/api/products/creator/my-products?page=${page}&limit=${state.pagination.limit}`);
      dispatch({
        type: 'FETCH_PRODUCTS_SUCCESS',
        payload: response.data
      });
